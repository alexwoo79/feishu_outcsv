...
# 员工的Cleveland dot plot
```{r}
#| warning: false
# Cleveland dot plot（Cleveland dot plot）：每月各用户总工时
user_hour_cleveland <- (function(df) {
  library(ggplot2)
  library(dplyr)
  library(forcats)
  library(showtext)
  ...
  p <- ggplot(df_prep, aes(x = 总工时, y = 姓名)) +
    geom_segment(aes(yend = 姓名), xend = 0, colour = "gray50") +
    geom_point(size = 2.2, colour = "#2c7fb8") +
    facet_wrap(~Month, ncol = 3, scales = "free_y") +
    labs(
      title = "Cleveland dot plot：每月各用户总工时",
      x = "总工时",
      y = "姓名"
    ) +
    theme_bw() +
